...
TimeWorked <- read_csv("Analysis Project/Capestone/Data/TimeWorked.csv")
...
DailySales<-read_csv("Analysis Project/Capestone/Data/DailySales.csv")
#Creation of the data set needed for the Analysis Where we have both hours worked and sales by departement
MotherOfAllData<-TimePerDay%>%inner_join(DailySales,by='Date')
#Removing NA from Total Sales, Bar Sales, Take out Sales and Poissonerie
MotherOfAllData[is.na(MotherOfAllData$Ventes_HT),]$Ventes_HT<-0
MotherOfAllData[is.na(MotherOfAllData$Bar),]$Bar<-0

#Removing unnecessary column
MotherOfAllData<-MotherOfAllData%>%select(-Weekday,-Month,-Day,-`annee fiscale`)
#Add a column for weekday and month
MotherOfAllData<-MotherOfAllData%>%mutate(Weekday=weekdays(Date),Month=months(Date))
#Add a column for payDay detection on the first and on the 15th of each month
MotherOfAllData<-MotherOfAllData%>%mutate(PayDay=ifelse(day(Date)==1 | day(Date)==15,TRUE,FALSE))
