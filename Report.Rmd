...
We first transform our train data to a time series and plot the sales over the years.we see that over the 3 years the data looks sationary, so we'll use a 7 days lag with a moving average window of 45 days to we define our model as:
...
```
We define our linear model such as all the variable defined in our train set is used to predict Sales:
```{r}
