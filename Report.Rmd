...
ts_train<-ts(zoo_train,start=c(2016,1),frequency=52)
zoo_test<-test_set%>%select(-Weekday,-Month,-Year,-EventDay,-Retail,-TakeOutSales,-Bar_Sales,-Sales_Restaurant)%>% read.zoo(.,format="%F")
ts_test<-ts(zoo_test,start=c(2019,1),end=c(2020,1),frequency=365)
```
# Modeling Data
## ARIMA Model
...
sw_augment(fit_arima, timetk_idx = TRUE) %>% 
  ggplot(aes(x = index, y = .resid)) +
    ...
    theme_classic()
```
We then forecast the first week of 2019
```{r}
...
fcast_tbl
actual_tbl <- DataForAnalysis %>% filter(Date >= '2018-12-31')
